...
- Assistant 응답을 markdown으로 렌더링(표/리스트/헤더 표시).
- 검색/생성 중 `Thinking` 진행 카드 실시간 표시.
...
3. 자연어로 질문
4. 검색/생성 중 `Thinking` 카드에서 진행 상태 확인
5. 응답의 소스 링크를 클릭해 원문 노트로 이동
6. `Open chat note` 버튼으로 현재 동기화된 스레드 노트 열기
